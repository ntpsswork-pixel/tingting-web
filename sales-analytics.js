...
// ─── Regex: strip product-code prefix ─────────────────────────
// Matches: "B+17-", "FDH05-", "H01-", "Moji01-", "B+01 - ", etc.
const CODE_PREFIX_RE = /^[A-Za-z()\s]*[A-Za-z+]+\d+[\s]*[-–][\s]*/u;

function stripProductCode(name) {
  if (!name) return '';
  return String(name).replace(CODE_PREFIX_RE, '').trim();
}

// ─── Known column aliases (Thai + English variants) ──────────
...
// ─── AI-assisted column mapper (fallback if heuristic fails) ──
...
Return: {"productCode":N_or_null,"productName":N,"group":N_or_null,"category":N_or_null,"avgPrice":N_or_null,"qty":N,"grossAmount":N_or_null,"discount":N_or_null,"netAmount":N,"branch":N}`;
  ...
  try {
    ...
    console.warn('AI mapping failed, using heuristic only', e);
    return null;
  }
}

// ─── Parse Excel file → structured rows ──────────────────────
async function parseExcelFile(file, showCodes = false) {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        // If critical fields missing → ask AI
        const hasCritical = mapping.productName !== undefined &&
                            mapping.qty !== undefined &&
                            mapping.netAmount !== undefined;
        if (!hasCritical) {
          const aiMap = await aiMapColumns(headers, dataRows);
          if (aiMap) {
            // Merge: AI fills in what heuristic missed
            for (const [k, v] of Object.entries(aiMap)) {
              if (v !== null && mapping[k] === undefined) mapping[k] = v;
            }
          }
        }
        ...
        const rows = dataRows.map(r => {
          const rawName = r[mapping.productName] ?? '';
          return {
            productCode : r[mapping.productCode] ?? '',
            productName : showCodes ? String(rawName) : stripProductCode(rawName),
            productNameRaw : String(rawName),
            group       : r[mapping.group] ?? '',
...
function aggregateData(rows) {
  ...
  // By category (strip TW./FD. prefix)
  function simplifyCategory(cat) {
    if (!cat) return 'อื่นๆ';
    let c = String(cat);
    for (const pfx of ['TW.','FD.','TW-','FD-']) {
      if (c.startsWith(pfx)) c = c.slice(pfx.length);
    }
  ...
  rows.forEach(r => {
    ...
    if (!prodMap[k]) prodMap[k] = { name: k, rawName: r.productNameRaw, revenue: 0, qty: 0, category: simplifyCategory(r.category) };
...
async function saveReportToFirestore(db, { fileName, period, summary, rows }) {
  ...
  await setDoc(reportRef, {
    ...
    uploadedBy  : window.currentUser?.uid || 'unknown',
    summary: {
    ...
    topProducts : summary.topProducts.slice(0, 50), // top 50 in main doc
  });

  // Save raw rows as subcollection (batched, 400 rows/doc)
  const BATCH_SIZE = 400;
  for (let i = 0; i < rows.length; i += BATCH_SIZE) {
    const chunk = rows.slice(i, i + BATCH_SIZE);
    await addDoc(collection(db, SA_COLLECTION, reportId, 'rawRows'), {
      rows: chunk, chunkIndex: Math.floor(i / BATCH_SIZE)
    });
  }
...
// ─── Firestore: load report list ─────────────────────────────
async function loadReportList(db, limit = 20) {
  const { collection, getDocs, query, orderBy, limit: fsLimit } =
    await import('https://www.gstatic.com/firebasejs/10.12.0/firebase-firestore.js');
  const q = query(
    collection(db, SA_COLLECTION),
