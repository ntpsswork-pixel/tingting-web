// ============================================================
//  TTGPlus — Sales Analytics Loader  (v2 — fixed)
...
window.openSalesAnalytics = async function() {
  ...
  if (location.hash !== '#sales-analytics') history.pushState({ nav: 'sales-analytics' }, '', '#sales-analytics');
};

function _saLoadScript(src) {
  return new Promise((res, rej) => {
    const existing = document.querySelector(`script[src="${src}"]`);
    if (existing) { setTimeout(res, 80); return; }
...
.sa-kpi:hover{background:#f1f5f9;}
.sa-kpi.active{background:#fffbeb;border-color:#f0b429;}
...
.sa-chip:hover{border-color:#f0b429;color:#0f172a;}
.sa-chip.active{background:#f0b429;border-color:#f0b429;color:#0f172a;font-weight:700;}
.sa-chip .sa-chip-x{font-size:10px;color:rgba(0,0,0,0.4);margin-left:2px;}
...
.sa-leg-item:hover{background:#f8fafc;}
.sa-leg-item.active{background:#fffbeb;}
    ...
    <div class="sa-card" style="margin-bottom:16px;">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;flex-wrap:wrap;gap:8px;">
        ...
        <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;">
          ...
          <label class="sa-btn primary" for="sa-file-input">⬆ เลือกไฟล์</label>
    ...
    <div id="sa-dashboard-content" style="display:none;">
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;gap:10px;flex-wrap:wrap;">
        ...
        <div style="display:flex;gap:7px;flex-wrap:wrap;">
          <button class="sa-btn" id="sa-btn-excel" onclick="_saExportExcel()">📥 Export Excel</button>
          <button class="sa-btn" id="sa-btn-pdf"   onclick="_saExportPDF()">🖨 Export PDF</button>
...
window._saProcessFile = async function(file) {
  if (!file.name.match(/\.(xlsx|xls|csv)$/i)) { window.toast('❌ รองรับเฉพาะ .xlsx, .xls, .csv','#c2410c'); return; }
  try {
    ...
    const { rows } = await SA.parseExcelFile(file, false); // always parse clean
...
window._saRenderDashboard = function(report) {
  ...
  document.getElementById('sa-kpi-container').innerHTML = `
    <div class="sa-kpi-grid">
      <div class="sa-kpi" onclick="_saKPIFilter('all')">
      ...
      <div class="sa-kpi" onclick="_saKPIFilter('qty')">
      ...
      <div class="sa-kpi" onclick="_saKPIFilter('product')">
        <div class="sa-kpi-label">สินค้า</div>
        <div class="sa-kpi-value">${(report.productCount||0).toLocaleString('th-TH')}</div>
        <div class="sa-kpi-sub">${report.branchCount} สาขา</div>
      </div>
      <div class="sa-kpi" onclick="_saKPIFilter('discount')">
  ...
  // Charts — render from full data first
...
window._saApplyFilters = function() {
  ...
  if (!r) return;
  const st = window._saState;
  st.filterSearch = (document.getElementById('sa-search')?.value || '').trim().toLowerCase();
  _saUpdateFilterSummary();
...
function _saGetFilteredData(report) {
  ...
  const rows = (report.rows || []).filter(r => {
    ...
    if (st.filterSearch) {
      ...
      if (!name.toLowerCase().includes(st.filterSearch) &&
          !r.category.toLowerCase().includes(st.filterSearch)) return false;
    }
    return true;
  });

  // Re-aggregate filtered rows
...
function _saRenderCharts(data, originalTotal) {
  const displayProducts = data.topProducts.map(p => ({
    ...p,
    name: window._saState.showCodes ? (p.rawName || p.name) : (p.cleanName || p.name)
  }));
  ...
  SACharts.renderTopProductsChart(displayProducts, 'sa-product-chart', window._saState.productMode);
  // Update donut total
...
window._saProductTab = function(mode, btn) {
  ...
  if (window._saCurrentReport) {
    ...
    const products = filtered.topProducts.map(p => ({
...
window._saSaveReport = async function() {
  ...
  try {
    ...
    const id = await SA.saveReportToFirestore(window.db, {
      fileName : r.fileName, period: r.period, summary: r, rows: r.rows || [],
    });
...
window._saToggleCompare = function(id, checked) {
  if (checked) window._saState.compareSet.add(id);
  else window._saState.compareSet.delete(id);
};
